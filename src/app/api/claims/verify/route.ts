...
export async function POST(request: Request) {
  try {
    ...
    const { data: consumed, error: consumeError } = await supabase.from("claim_challenges").update({ used_at: new Date().toISOString() }).eq("nonce", body.nonce).is("used_at", null).select("nonce").maybeSingle();
    if (consumeError || !consumed) return NextResponse.json({ error: "Claim proof was already used. Request a new claim." }, { status: 409 });
    const { error: claimError } = await supabase.from("agent_claims").upsert({ agent_id: challenge.agent_id, chain_id: 56, registry_address: agent.contract_address.toLowerCase(), owner_address: body.address.toLowerCase(), signature: body.signature, verified_at: new Date().toISOString(), revoked_at: null }, { onConflict: "chain_id,registry_address,agent_id" });
