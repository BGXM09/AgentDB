...
export default async function AgentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="container page-content"><div className="breadcrumb"><Link href="/">Home</Link><span>/</span><Link href="/agents">Agents</Link><span>/</span>Agent #{id}</div>
    ...
    <div className="overview-grid"><section className="panel summary-panel"><div className="panel-title"><h2>Overview</h2></div><dl className="summary-list"><div><dt>AgentDB Score</dt><dd><details className="score-details"><summary>{score.score == null ? "Insufficient history" : `${score.score} / 100 · ${score.confidence}`}</summary><div className="score-popover">{score.evidence.map((item) => <p key={item.label}><b>{item.label} ({item.weight})</b><span>{item.value}</span></p>)}</div></details></dd></div><div><dt>Category</dt><dd>{normalized.derived.category === "Other" ? "Unclassified" : normalized.derived.category} <small>({normalized.derived.categoryConfidence} confidence)</small></dd></div><div><dt>Reputation</dt><dd>{agent.total_feedbacks ? `${agent.average_score} average · ${agent.total_feedbacks} feedback` : "No ERC-8004 feedback"}</dd></div><div><dt>Endpoint health</dt><dd>{agent.health_score == null ? "No health evidence" : `${agent.health_score}/100`}</dd></div><div><dt>Commerce</dt><dd>{normalized.derived.commerce === "unknown" ? "No verified commerce interface" : normalized.derived.commerce.toUpperCase()}</dd></div></dl></section><section className="panel summary-panel"><div className="panel-title"><h2>More Info</h2></div><dl className="summary-list"><div><dt>Network</dt><dd>BNB Smart Chain (56)</dd></div><div><dt>Owner</dt><dd><code>{agent.owner_address}</code></dd></div><div><dt>Agent wallet</dt><dd><code>{typeof agent.agent_wallet === "string" ? agent.agent_wallet : "Not declared"}</code></dd></div><div><dt>Protocols</dt><dd>{agent.supported_protocols?.join(", ") || "None declared"}</dd></div><div><dt>Updated</dt><dd>{absoluteDate(agent.updated_at)}</dd></div></dl></section></div>
