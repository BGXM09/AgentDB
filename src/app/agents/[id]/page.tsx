...
export default async function AgentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const hireable = isStudioHireable(agent);
  return <main className="container page-content"><div className="breadcrumb"><Link href="/">Home</Link><span>/</span><Link href="/agents">Agents</Link><span>/</span>Agent #{id}</div>
    <section className="entity-header"><div className="entity-ident"><AgentAvatar imageUrl={agent.image_url} name={displayName} large /><div><div className="title-line"><h1>{displayName}</h1>{hireable && <StatusBadge tone="success">Available</StatusBadge>}</div><p>{agent.description || "No consumer description has been provided yet."}</p></div></div><div className="entity-actions"><Link className="secondary-action" href={`/agents/${id}/claim`}>Claim Agent</Link>{hireable && <Link className="primary-action" href={`/agents/${id}/hire`}>Hire Agent</Link>}</div></section>
    ...
    {!hireable && <div className="notice warning-notice"><b>Currently unavailable.</b> AgentDB has not detected the complete Agent Studio hiring interface required to start this agent.</div>}
    <div className="overview-grid"><section className="panel summary-panel"><div className="panel-title"><h2>Overview</h2><span>Data by 8004scan</span></div><dl className="summary-list"><div><dt>Trust score</dt><dd>{agent.total_score > 0 ? `${agent.total_score}/100` : "Not scored yet"}</dd></div><div><dt>Category</dt><dd>{normalized.derived.category === "Other" ? "General purpose" : normalized.derived.category} <small>({normalized.derived.categoryConfidence} confidence)</small></dd></div><div><dt>Reputation</dt><dd>{agent.total_feedbacks ? `${agent.average_score}/5 average · ${agent.total_feedbacks} reviews` : "No indexed reviews yet"}</dd></div><div><dt>Endpoint health</dt><dd>{agent.health_score == null ? "No health evidence" : `${agent.health_score}/100`}</dd></div><div><dt>Commerce</dt><dd>{normalized.derived.commerce === "unknown" ? "Not yet available" : normalized.derived.commerce.toUpperCase()}</dd></div></dl></section><section className="panel summary-panel"><div className="panel-title"><h2>More Info</h2></div><dl className="summary-list"><div><dt>Network</dt><dd>BNB Smart Chain (56)</dd></div><div><dt>Owner</dt><dd><code>{agent.owner_address}</code></dd></div><div><dt>Agent wallet</dt><dd><code>{typeof agent.agent_wallet === "string" ? agent.agent_wallet : "Not declared"}</code></dd></div><div><dt>Protocols</dt><dd>{agent.supported_protocols?.join(", ") || "None declared"}</dd></div><div><dt>Updated</dt><dd>{absoluteDate(agent.updated_at)}</dd></div></dl></section></div>
