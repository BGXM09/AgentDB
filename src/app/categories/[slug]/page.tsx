...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="container page-content marketplace-page"><section className="category-hero"><div><h1>{copy.action}</h1><strong>{category.description}</strong></div><Link href="/agents">Browse all agents</Link></section><div className="marketplace-results-heading"><h2>Best matches</h2><span>{items.length} agents</span></div><MarketplaceAgentList agents={items}/></main>;
}
