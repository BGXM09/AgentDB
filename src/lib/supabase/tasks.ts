...
export async function listMarketplaceTasks(limit = 50): Promise<MarketplaceTask[] | null> {
  try {
    const { data, error } = await createServiceSupabase().from("marketplace_tasks").select("*").eq("chain_id", 56).order("created_at", { ascending: false }).limit(limit);
